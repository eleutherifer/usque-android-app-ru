...
- заменена конечная точка по умолчанию на 360tv.ru:443
- китайский язык заменён на русский
- обновлена библиотека Usque на версию v4.2.1
- добавлен режим HTTP/2
- добавлен режим Авто
- добавлено отображение лога
- добавлены возможности заменить или удалить лицензионный ключ
- добавлена возможность импортировать в файл и экспортировать из файла все настройки
...
```text
https://github.com/eleutherifer/usque-android-app-ru/releases/download/v4.2.1.8/usque-android-app-ru-release-v4.2.1.8.apk
```

> The GitHub Actions workflow reads `versionName` from `app/build.gradle` and publishes the APK to the matching versioned Release, for example `4.2.1.8` → `v4.2.1.8`.
...
|-----------|-------------------|-------------|
| SNI | `360tv.ru` | TLS SNI value used by the tunnel |
